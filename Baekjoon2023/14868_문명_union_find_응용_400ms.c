...
int _flood(int* arr, const int N, int x, int y, int value) {
	if (x < 1 || x > N || y < 1 || y > N || arr[xy2i(x, y)] != -1) return 0;
	arr[xy2i(x, y)] = value;
	int a = -1;
	for (int i = 0; i < 4; i++) { a += _flood(arr, N, x + dx[i], y + dy[i], value); }
	return a;
}
int solve14868(const int N, int K) {
	//ArrayQueue* queue = AQ_new((N + 1) * (N + 1));
	pair_t* queue = calloc((N + 1) * (N + 1), sizeof(pair_t)); if (!queue) exit(1);
	int queue_f = 0;
	int queue_r = 0;
	int* visited = calloc((N + 1) * (N + 1) + 1, sizeof(int)); if (!visited) exit(1); // also contain Group num.
	int visited_total = 0;
	int answer = 0;
	int* disjset = UF_init(K + 1);
	int disjset_size = 0;
	for (int i = 0; i < K; i++) {
		pair_t xy;
		scanf("%d%d", &xy.x, &xy.y);
	...
	for (int i = 0; i < K; i++) {
		int x = queue[i].x;
		int y = queue[i].y;
		if (visited[xy2i(x, y)] != -1) continue;
		disjset_size++;
		disjset[disjset_size] = _flood(visited, N, x, y, disjset_size); // 1, 2, 3 ...
	}
