...
int UF_union(int* roots, int A, int B) {
	A = UF_find(roots, A);
	B = UF_find(roots, B);
	if (A == B) return 0;
	roots[B] = A;
...
void _flood(int x, int y, int value) {
	if (x < 1 || x > N || y < 1 || y > N || visited[xy2i(x, y)] != -1) return;
	visited[xy2i(x, y)] = value;
	queue[queue_r++] = (pair_t){ x, y };
	for (int i = 0; i < 4; i++) { _flood(x + dx[i], y + dy[i], value); }
}
int solve14868() {
	int answer = 0;
	int disjset_size = 0;
	int* disjset = NULL;
	if (!(visited = calloc((N + 1) * (N + 1), sizeof(int)))) exit(1);
	if (!(queue = calloc((N + 1) * (N + 1), sizeof(pair_t)))) exit(1);
	queue_f = 0;
	queue_r = 0;
	for (int i = 0; i < K; i++) {
		pair_t xy;
	...
	for (int i = 1; i <= N; i++) for (int j = 1; j <= N; j++) {
		...
		disjset_size++;
		_flood(i, j, disjset_size); // 1, 2, 3 ...
	}
	if (!(disjset = calloc(K + 1, sizeof(int)))) exit(1);
	memset(disjset, -1, sizeof(int) * (K + 1));

	while (disjset_size > 1) {
		for (int i = queue_r - queue_f; i >= 1; i--) {
			...
			for (int j = 0; j < 4; j++) {
				...
				for (int k = 0; k < 4; k++) {
					...
					if (UF_union(disjset, disj_num1, disj_num2)) disjset_size--;
				}
			}
			if (disjset_size == 1) break;
		}
		answer++;
	}
	free(queue);
